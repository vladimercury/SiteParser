class WordParserUtil:
    ...
    def get_stemmed_words_without_stopwords(text):
        import nltk
        words = WordParserUtil._get_words_without_stopwords(text)
    ...
    @staticmethod
    def _get_words_without_stopwords(text):
        import nltk
        words = WordParserUtil._get_all_words(text)
        stopwords = []
        for i in ['russian', 'english']:
            stopwords += nltk.corpus.stopwords.words(i)
        return [word for word in words if word[0] not in stopwords]